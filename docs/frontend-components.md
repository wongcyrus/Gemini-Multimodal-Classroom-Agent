...
flowchart TD
    ...
    subgraph TeacherFlow [Teacher Workspace]
        ...
        CV --> TAB2[VideoLibrary.jsx - Recorded MP4s & LectureRecordingsView]
        CV --> TAB3[AttendanceView.jsx - 3-State Heatmaps & BingoResultsView]
        CV --> TAB4[IrregularitiesView.jsx - AI Audit Evidence]
        CV --> TAB5[TasksManagementView.jsx - Practical Tasks & Grading Matrix]
        CV --> TAB6[ClassManagement.jsx - Roster & Settings]
...
*   **`ClassManagement.jsx`**: A comprehensive component that allows teachers to create new classes and manage existing ones. Features configurable **AI Monitoring Modes** (`⚡ Client AI + Fallback`, `💻 Client AI Only`, `☁️ Cloud AI Only`, `🚫 AI Disabled`), customizable gaze sensitivity thresholds (Yaw/Pitch angles and debounce duration), configurable **Bingo Active Presence Retry Grace Delay** (`1m`, `2m`, `3m default`, `5m`, `10m`) controlling the schedule delay before Google Cloud Tasks dispatches a Strike 2 follow-up verification, configurable **Default Capture Mode** (`dual`, `screen`, `webcam`), dedicated **Exam & Test Periods (Restricted from Students)** manager (`examPeriods`) allowing instructors to define specific date and time ranges for 1–2 semester exams where recordings are completely withheld from students while remaining 100% accessible to instructors for auditing, configurable **Student Screen Recording Access Policy** (`always_enabled`, `disabled`, `delayed_release`), **Cross-Class Student Profile Propagation** integrating real-time Institutional Student Directory (`/studentDirectory`) caching and live auto-enrichment (`✨ Directory` badge, `✨ X auto-filled from other classes` counter), structured **Batch Student Import (`BatchStudentUploadModal`)** supporting unified `Student Name` format, and one-click **Roster Export (Excel)** (`Class_{id}_Roster.xlsx`) with auto-merged institutional profile records.
*   **`BatchStudentUploadModal.jsx`**: A dedicated modal dialog for bulk-importing student profiles into a class. Provides a 1-click **Download Excel Template** button (`student_roster_template.xlsx`) and accepts genuine Excel files (`.xlsx` or `.xls`) with standard columns (`StudentEmail,StudentName,Nickname,Programme,Class`). Full Unicode preservation guarantees that Chinese names and nicknames (`大文`, `陳大文`, `阿欣`) load without garbling. Validates institutional email domains (`@stu.vtc.edu.hk`), provides a real-time table preview with validation warnings, merges existing profile records from the institutional directory, and applies changes directly to the class roster.
...
*   **`MonitorView.jsx`**: Provides a real-time grid view of all students during a live session. Features:
    ...
    *   **Zero-Space Compliance Filter Dropdown**: Real-time filtering by issue category (`👥 All Students`, `⚠️ Problems`, `📷 Missing Cam`, `🎙️ Missing Mic`, `🖥️ Not Sharing`, `🚨 AI Alerts`) with dynamic student counts and resolved student display names.
    ...
    *   **Quick Excel Audit Export**: Instant one-click export (`📥 Export Excel`) capturing the currently filtered student compliance state, detected irregularities, stream flags, and gaze vector data into a timestamped `.xlsx` file with full student profiles.
...
*   **`ControlsPanel.jsx`**: The consolidated sidebar control center for teachers during live monitoring. Reorganized in a logical top-down hierarchy:
  ...
  4. **👥 Attendance & Status**: 1-click "Not Sharing" student counter/modal (with resolved student display names and cohort tags) and Attendance Excel export.
...
*   **`PerformanceAnalyticsView.jsx`**: Provides analytics and visualizations of student performance data. Features:
  ...
  * **Dual Excel Export Capabilities**: Includes a top-level **`📥 Export Excel`** button for the full class milestone report, plus a dedicated **`📥 Export Matrix Excel ({count})`** button directly in the table header to export the actively sorted and filtered student cohort with complete student profiles.
*   **`AttendanceView.jsx`**: Provides a comprehensive view of student attendance and AI analysis for a selected lesson. It displays a unified table showing screen share attendance (total minutes, percentage, and a per-minute heatmap), alongside AI-estimated working minutes and percentage. Features:
    ...
    *   **Export Options**: Exports the full multi-metric roster including student display names, email, cohort, programme, attended minutes, deducted minutes, working time, and individual per-minute status codes directly to Excel (`.xlsx`). Includes student-specific AI summary modal.
*   **`VideoAnalysisJobs.jsx`**: Master dashboard for tracking, retrying, and synthesizing video analysis jobs.
  ...
  * **Multi-Tier Export Capabilities**:
    * **Level 1 Batch Log**: Export full job history to Excel (`Export Jobs Log (Excel)`).
    * **Level 2 Full Batch Export**: Export all student findings to Excel (`📥 Export Findings (Excel)`) or JSON (`📦 Export Batch (JSON)`).
    * **Level 2 Filtered Export**: Export only actively searched/filtered students via header toolbar button (`📥 Export Excel ({count})`).
...
*   **`AiJobsTable.jsx`**: A table used within `VideoAnalysisJobs.jsx` to display individual AI jobs with status, model used, token metrics, multi-attribute media path resolution, and computed cost ($ USD) via `formatAiCost`. Features student display names with cohort badges and a dedicated **Actions** column with 1-click **`Excel`**, **`JSON`**, and **`View`** buttons for direct inspection and export per student.
...
*   **`JobResultModal.jsx`**: Modal overlay for inspecting completed AI jobs with multi-format downloads (**`📥 Excel`**, **`📥 JSON`**, **`📝 Markdown`**, and **`📄 Text Report`**), resolved student display names, one-click clipboard copying, and failed job error traceback inspector.
...
*   **`aiCostCsvExporter.js` (Utility)**: Converts aggregated AI financial data and itemized job records into formatted Excel spreadsheets (`exportAiCostToExcel`) with automatic browser downloads and full student profile enrichment.
*   **`studentCompliance.js` (Utility)**: Pure domain utility evaluating real-time student stream states, hardware sharing flags, and gaze orientation against class rules (`evaluateStudentCompliance`, `getComplianceSummary`, `filterStudentsByCompliance`, `getNudgeMessageForFilter`, `exportComplianceResultsToExcel`).
*   **`studentDisplayUtils.js` (Utility)**: Unified student profile resolution engine (`resolveStudentInfo`, `formatStudentDisplay`, `getStudentBadge`) resolving student display names (e.g. `大文 (Chan Tai Man)`), email, class/cohort, and programme across all tables, cards, and export functions.
...
## Practical Tasks & Google Drive Archival

*   **`TasksManagementView.jsx`**: The teacher's central hub for managing hands-on lab exercises and technical assignments within a class. Displays task cards with max score, attempt limits, rubric step count, submission counts, and status filters (`all`, `active`, `archived`). Features 1-click navigation to create/edit tasks or open the grading matrix.
*   **`TaskEditorModal.jsx`**: Full-featured modal for drafting and configuring lab tasks. Enables instructors to specify task parameters (title, description, max score, time limits, allowed attempts) and author rubric milestones. Includes **"✨ Extract Steps from Demo"** which analyzes the teacher's reference screen recording to automatically generate rubric steps using Gemini.
*   **`TaskGradingMatrixView.jsx`**: Comprehensive grading workspace for practical tasks. Provides:
    *   **Top Workspace Toolbar**: Dynamic `☁️ Backup Task Videos (N)` button displaying the count of compiled student recordings ready for Google Drive archival, alongside an interactive Drive destination breadcrumb path (`[Base Folder] / [Class] / Tasks / [Task Title]`).
    *   **Roster & Submission Table**: Displays student name, email, cohort, status, attempts, score (with manual override), individual rubric step scores, and a dedicated **Video / Drive** column.
    *   **Video / Drive Column**: Direct external link `📁 Drive ↗` for archived videos, one-click `☁️ Backup` for single student videos, and `▶️ Watch` button for in-browser screencast inspection.
    *   **In-Browser HTML5 Video Player Modal**: Displays the student's exact lab attempt screencast with native playback controls without page reloads or loss of table state.
    *   **Inspect Modal Integration**: Detailed rubric milestone breakdown with status pills (`completed`, `partial`, `missed`), student work duration, and live Google Drive cloud backup status card.
    *   **Excel Gradebook Export**: Leverages `exportTaskGradingToExcel` to download formatted spreadsheets including student profiles, rubric breakdown, and direct Google Drive viewing hyperlinks.
*   **`DriveBackupProgressModal.jsx`**: Reusable animated progress dialog for Google Drive batch uploads. Features dual progress bars (Overall Batch % and Current File Upload %), real-time student upload status log, file size telemetry, and an **AbortController** cancel button that immediately terminates active HTTP streams.
*   **`StudentTaskWorkspaceModal.jsx`**: Dedicated student workspace modal for undertaking hands-on lab tasks. Enforces screen recording during the attempt, displays active countdown timer and rubric guidelines, and compiles/submits the screencast upon completion.
*   **`StudentTaskFeedbackView.jsx`**: Student-facing feedback card displaying final score, AI evaluation report, itemized milestone statuses, teacher manual overrides, and review comments.
*   **`lessonUtils.js` (Utility)**: Centralized 3-tier lesson naming resolution engine. Formats two-digit zero-padded lesson indices (`Lesson 01`), matches timestamps against class schedule timetable slots ($\pm 30$-minute fuzzy buffer), sanitizes illegal filesystem and Google Drive folder characters, routes task submissions to `Tasks / [Task Title]`, and formats dropdown options.
*   **`googleDriveService.js` (Utility)**: 100% client-side Google Drive API service using least-privilege `drive.file` scope. Executes entirely in the instructor's browser via direct HTTP/REST calls (`https://www.googleapis.com`), completely bypassing Cloud Functions to eliminate server egress bandwidth and timeout bottlenecks. Implements in-memory folder lookup caching (`Map<string, folderObj>`), recursive folder hierarchy resolution (`[Base] / [Class] / [Lesson or Tasks] / ...`), and chunked resumable upload streaming via browser `XMLHttpRequest` with upload progress events.
*   **`useGoogleDrive.js` (Custom Hook)**: Client-side lifecycle hook managing Google Drive OAuth tokens strictly within browser volatile memory (`sessionStorage`), customizable base folder state with `localStorage` persistence, teacher lecture uploads (`uploadRecording`), bulk student session video backups (`backupStudentVideosToDrive`), and practical task video backups (`backupTaskVideosToDrive`) with bidirectional Firestore metadata persistence. Streams video blobs directly from Firebase Storage to Google Drive using client RAM.
...
*   **`AudioTranscriptModal.jsx`**: Dialogue playback modal for teachers displaying multi-speaker turns with resolved student names, synchronized webcam snapshots, clickable seek buttons, and Excel/TXT transcript exports.
