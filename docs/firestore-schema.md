...
2. [Collections](#collections)
   ...
   - [`passkeyAuditLogs`](#passkeyauditlogs)
   - [`passkeyPairingTokens`](#passkeypairingtokens)
   - [`progress`](#progress)
...
Stores temporary (10-minute) cryptographic pairing session tokens generated on the student's logged-in Lab PC and scanned via QR code on mobile to pair biometric passkeys without typing passwords.

*   **Document ID**: `tokenId` (UUID string).
*   **Fields**:
    *   `tokenId`: (string) Random cryptographic pairing token identifier.
    *   `studentUid`: (string) UID of the student who initiated pairing on the Lab PC.
    *   `studentEmail`: (string) Email of the student.
    *   `classId`: (string, optional) Class context identifier.
...
Stores physical WebAuthn (FIDO2) public key credentials for enrolled students, enforcing a cryptographic **1-Phone = 1-Student hardware lock** to prevent password-sharing human proxy attendance in computer labs without webcams.

*   **Document ID**: `studentUid` (string, the student's unique user identifier).
*   **Security**: Read and write restricted to server Cloud Functions and the authenticated student. Credential uniqueness is verified on registration.
*   **Fields**:
    *   `studentUid`: (string) UID of the student.
    *   `studentEmail`: (string) Institutional email of the student.
    *   `credentialID`: (string) Base64 WebAuthn platform authenticator credential ID, guaranteed globally unique per physical device.
    *   `credentialPublicKey`: (string) Base64 encoded ECDSA / COSE public key generated by the phone's Secure Enclave / Titan chip.
    *   `counter`: (number) Signature counter incremented on every biometric authentication to prevent replay attacks.
    *   `deviceModel`: (string) Hardware device description (e.g., `'iPhone 16 Pro'`, `'Pixel 9 Pro'`).
    *   `transports`: (array) Transport mechanisms supported by the authenticator (e.g. `['internal']`).
    *   `registeredAt`: (timestamp) Server timestamp when the device was first paired.
    *   `updatedAt`: (timestamp) Server timestamp when last updated or verified.
